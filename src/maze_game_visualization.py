...
class MazeGamePygameVisualization:
    ...
    def draw_main_menu(self):
        ...
        single_play_img = pygame.image.load("img/playButton.png").convert_alpha()
        twitch_play_img = pygame.image.load("img/twitchButton.png").convert_alpha()
        ...
        self.draw_text("Menu Screen", self.text_color, self.screen_width//2 - 145 , 50)
    ...
    def draw_game_over(self):
        ...
        pygame.draw.rect(self.screen, pygame.Color("white"), pygame.Rect(self.screen_width//2 - 350//2, self.screen_height//2 - 100//2, 350, 100), 0, 10)
        
        img = self.font.render("Game Over", True, pygame.Color("Blue"))
        text_width, text_height = img.get_width(), img.get_height()
        self.screen.blit(img, (self.screen_width//2 - text_width//2, self.screen_height//2 - text_height//2))


        img = self.small_font.render("Press Enter to restart", True, pygame.Color("Blue"))
        text_width, text_height = img.get_width(), img.get_height()
        self.screen.blit(img, (self.screen_width//2 - text_width//2, self.screen_height//2 - text_height//2 + 30))


    def draw_tile(self, board_x: int, board_y: int , tile: Tile):
        x, y = tile.col*tile.width, tile.row*tile.height
        pygame.draw.rect(self.screen, tile.tile_color, pygame.Rect(board_x + x, board_y + y, tile.width, tile.height), 0)
        pygame.draw.rect(self.screen, tile.border_color, pygame.Rect(board_x + x, board_y + y, tile.width, tile.height), 3)
    ...
    def draw_maze(self, maze: MazeGameLayer):
        ...
        for i in range(row):
            for j in range(col):
                ...
                tile = Tile(i, j, tile_width, tile_height, tile_color, tile_border)
                self.draw_tile(50, 50, tile)
        pygame.draw.rect(self.screen, pygame.Color("black"), pygame.Rect(50, 50, col*tile_width, row*tile_height ), 3)
